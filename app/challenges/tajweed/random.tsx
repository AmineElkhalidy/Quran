...
export default function TajweedChallenge() {
  ...
  const [question, setQuestion] = useState<TajweedQuestion | null>(null);
  const [prevAyah, setPrevAyah] = useState<string | null>(null);
  ...
  const nextQuestion = useCallback(() => {
    let randomQ = TAJWEED_RULES[Math.floor(Math.random() * TAJWEED_RULES.length)];
  ...
  const parts = question.ayahContext.split(question.highlightedWord);

  return (
    <>
      <Stack.Screen options={{ title: 'أحكام التجويد', headerTitleAlign: 'center' }} />
      <View style={styles.container}>
        <View style={styles.header}>
        <Text style={styles.title}>أحكام التجويد</Text>
        <View style={styles.streakRow}>
          <Text style={styles.subtitle}>ما هو الحكم التجويدي في الكلمة الملونة؟</Text>
          {streak > 0 && (
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: Typography.heading2, fontWeight: 'bold', color: Colors.tajweed, textAlign: 'right' },
  ...
  highlightedWord: { color: Colors.tajweed, fontWeight: 'bold' },
  explanationText: {
  ...
  nextBtn: { width: '100%', backgroundColor: Colors.tajweed, padding: Spacing.md, borderRadius: BorderRadius.md, alignItems: 'center' },
